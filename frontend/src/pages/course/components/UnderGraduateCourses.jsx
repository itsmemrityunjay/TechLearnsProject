...
const UnderGraduateCourses = () => {
    ...
    const fetchCourses = useCallback(async () => {
        ...
        try {
            ...
            // Method 1: If your backend supports OR operations
            queryParams.append('courseFor', 'undergraduate,both,null');
            queryParams.append('courseForNot', 'elementary'); // Exclude elementary courses

            // Alternative Method 2: If your backend doesn't support complex queries
            // Don't filter by courseFor on the server side, we'll filter client-side
            // Note: Uncomment this section and comment out the previous if needed
            /*
            ...
            */
            ...
            fetchedCourses = fetchedCourses.filter(course =>
                course.courseFor === 'undergraduate' ||
    ...
    const handleShowToggle = () => {
        if (visibleCount === 4) {
            ...
            document.getElementById('undergraduate-courses')?.scrollIntoView({ behavior: 'smooth' });
        }
    };
    ...
    return (
        <section id="undergraduate-courses" className="py-12 bg-gray-50">
            <div className="container mx-auto px-4">
                {/* Header and filters */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                    <div>
                        <div className="flex items-center mb-2">
                            <FaGraduationCap className="text-[#f99e1c] mr-2" size={24} />
                            <h2 className="text-2xl md:text-3xl font-bold text-[#013954]">
                                Courses for Under Graduates
                            </h2>
                        </div>
                        <div className="mt-2 w-20 h-1 bg-[#f99e1c]"></div>
                        <p className="mt-3 text-gray-600 max-w-xl">
                            Comprehensive degree-level courses designed to enhance academic knowledge and prepare students for professional careers.
                        </p>
                    </div>
                ...
                {!loading && !error && courses.length === 0 && (
                    <div className="text-center py-10">
                        <p className="text-gray-500">No undergraduate courses found with the selected filters.</p>
                        <button
                            className="mt-4 px-4 py-2 bg-[#013954] text-white rounded-md"
                            onClick={resetFilters}
                        >
