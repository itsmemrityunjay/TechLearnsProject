...
const PastCompetitions = () => {
    ...
    // Fetch competitions from API - only past competitions (ended)
    useEffect(() => {
        const fetchCompetitions = async () => {
            try {
                ...
                // Filter to only include competitions whose end date is in the past
                const pastCompetitions = data.filter(competition => {
                    const endDate = new Date(competition.endDate);
                    return endDate < currentDate;
                });

                setCompetitions(pastCompetitions);
            } catch (error) {
                console.error('Error fetching past competitions:', error);
                setError('Failed to load past competitions. Please try again later.');
            } finally {
                setLoading(false);
            }
        };
            ...
            <div className="container mx-auto px-4 md:px-6">
                ...
                <div className="mb-10 text-center">
                    ...
                    <p className="mt-4 max-w-2xl mx-auto text-gray-600">
                        Browse through our completed competitions and view results.
                    </p>
                </div>
                ...
                {!loading && !error && competitions.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <h3 className="text-xl font-semibold text-gray-500 mb-1">No past competitions found</h3>
                        <p className="text-gray-400">Check back later for completed competitions</p>
                    </div>
                )}

                {/* Past Competitions grid - responsive: 1 column on mobile, 2 on tablet, 4 on desktop */}
                {!loading && !error && competitions.length > 0 && (
                            ...
                            {visibleCompetitions.map((competition, index) => (
                                ...
                                >
                                    ...
                                    <div className="relative h-48 overflow-hidden">
                                        ...
                                        />
                                        <div className="absolute top-0 left-0 bg-gray-500 text-white px-3 py-1 text-sm font-medium rounded-br-lg">
                                            Completed
                                        </div>
                                    ...
                                    <div className="p-5">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <Link
                                                ...
                                                target="_blank"
                                                className="bg-[#013954] hover:bg-[#012d44] text-white text-center px-4 py-2 rounded-md text-sm font-medium shadow-sm transition-colors duration-200"
                                            >
                                                Show Details
                                            </Link>

                                            <button
                                                className="bg-[#f99e1c] hover:bg-[#e08200] text-white px-4 py-2 rounded-md text-sm font-medium shadow-sm transition-colors duration-200 flex items-center justify-center"
                                            >
                                                <FaChartLine className="mr-1" /> Results
                                            </button>
                                        </div>
                ...
                {/* Show More/Less buttons - only show if there are competitions */}
                {!loading && !error && competitions.length > 0 && (
